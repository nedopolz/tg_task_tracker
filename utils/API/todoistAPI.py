...
class ToDoist(BaseAPI):
    ...
    async def create_todo(self, todo: ToDoItem) -> bool | APIException:
        await self.todoist.add_task(content=todo.name, labels=[self.enum_to_color(todo.color)],
                                    project_id=self.project_id)
        return True

    async def get_id_by_name(self, name: str) -> str:
        name = name.replace(",", "")
        item = await self.todoist.get_tasks(**{"filter": f"search: {name} & created: today"})
        if len(item) > 0:
            return item[0].id
        return ''
    ...
    async def list_todo(self) -> list[ToDoItem | None] | APIException:
        ...
        for item in items:
            ...
            todo = ToDoItem(color=self.color_to_enum(color), name=item.content, is_done=False)
            results.append(todo)
        return results
